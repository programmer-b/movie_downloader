...
import 'package:movie_downloader/Commons/kf_functions.dart';
import 'package:movie_downloader/Commons/kf_strings.dart';
import 'package:movie_downloader/Commons/kf_themes.dart';
import 'package:movie_downloader/Components/kf_web_component.dart';
...
class _KFVideoLoadingComponentState extends State<KFVideoLoadingComponent> {
  ...
  bool get webAccessedOnce => getBoolAsync(keyWebAccessedOnce);
  ...
  void setWebAccessed() async => webAccessedOnce
      ? await setValue(keyWebAccessed, true)
      : await setValue(keyWebAccessedOnce, true);

  Widget _configureWeb(String title, String rootImagePath) => FutureBuilder(
      future: 20.seconds.delay,
      builder: (_, snap) {
        if (snap.ready) {
